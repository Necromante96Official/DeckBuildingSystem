...
(() => {
  ...
  function manualTierForCard(card, manualByEffectKey) {
    if (!manualByEffectKey || !card.desc_pt) return null;
    return manualByEffectKey.get(effectTextKey(card.desc_pt)) ?? null;
  }
  function exportDeckToExcel(entries, bySlug, summary, manualByEffectKey, filename = "deck-forbidden-legacy.xlsx") {
    ...
    const wsCards = utils.json_to_sheet(rows);
    utils.book_append_sheet(wb, wsCards, "Cartas");
    const summaryRows = [
      ...
      { Campo: "Tier m\xE9dio", Valor: summary.average_power_tier },
      ...summary.seed != null ? [{ Campo: "Seed", Valor: summary.seed }] : [],
  ...
  function canAddCopy(card, state2, opts) {
    ...
    if (!opts?.allowOverSize && state2.total >= DECK_SIZE) return false;
  ...
  function entriesToDeckState(entries, bySlug, opts) {
    const state2 = createDeckState();
    for (const e of entries) {
      const card = bySlug.get(e.slug);
      if (!card) continue;
      for (let i = 0; i < e.copies; i++) {
        addCopy(card, state2, opts);
      }
    }
    return state2;
  }
  function removeAllCopies(card, state2) {
    const current = state2.counts.get(card.slug) || 0;
    if (current < 1) return false;
    for (let i = 0; i < current; i++) {
      removeCopy(card, state2, { protectSupportMins: false });
    }
    return true;
  }
  function setCopyCount(card, state2, target, opts) {
    ...
    if (clamped > current) {
      for (let i = current; i < clamped; i++) {
        if (!addCopy(card, state2, opts)) return false;
      }
      return true;
    }
    for (let i = current; i > clamped; i--) {
      if (!removeCopy(card, state2, { protectSupportMins: false })) return false;
    }
    return true;
  }
  ...
  function resolveTheme(index, requestedIds) {
    ...
    if (unique.includes("livre") || unique.length === 0) {
      ...
      return { arch: livre, isLivre: true, activeIds: /* @__PURE__ */ new Set(["livre"]) };
    }
    if (unique.length === 1) {
  ...
  function mentionsArchetype(card, arch) {
    ...
    const text = cardText(card);
    const keys2 = [arch.label, arch.id.replace(/_/g, " "), ...arch.synergy_tipos];
    if (arch.id.includes("red_eyes")) keys2.push("red-eyes", "red eyes");
    if (arch.id.includes("blue_eyes")) keys2.push("blue-eyes", "blue eyes");
    if (arch.id.includes("dark_magician")) {
      keys2.push("dark magician", "dark magic");
    }
    if (arch.id.includes("black_luster")) {
      keys2.push("black luster", "super soldier", "envoy of the beginning");
    }
    return keys2.some((k) => k && text.includes(norm(k)));
  }
  function mentionsForeignArchetype(card, arch, allEntries, activeArchIds) {
    const text = cardText(card);
    for (const other of allEntries) {
      if (other.livre || other.id === arch.id) continue;
      if (activeArchIds?.has(other.id)) continue;
      const keys2 = [other.label, other.id.replace(/_/g, " ")];
      if (other.id.includes("red_eyes")) keys2.push("red-eyes", "red eyes");
      if (other.id.includes("blue_eyes")) keys2.push("blue-eyes", "blue eyes");
      if (other.id.includes("dark_magician")) {
        keys2.push("dark magician", "dark magic");
      }
      if (other.id.includes("black_luster")) {
        keys2.push("black luster", "envoy of the beginning");
      }
      if (keys2.some((k) => k && text.includes(norm(k)))) {
  ...
  function buildPool(cards, arch, allEntries, monsterTipo, monsterAtributo, activeArchIds) {
  ...
  function buildMergedPool(cards, ids, allEntries, monsterTipo, monsterAtributo) {
    const active = new Set(ids);
    const coreMap = /* @__PURE__ */ new Map();
    const supportMap = /* @__PURE__ */ new Map();
    const fillerMap = /* @__PURE__ */ new Map();
    for (const id of ids) {
  ...
  function synergyScore(candidate, deckCards, arch, coreSet, otherArch, core, target, allEntries, themeTipo, themeAtributo, activeArchIds) {
    let score = 0;
  ...
  function generateDeck(options) {
    ...
    const pool = isLivre ? buildLivrePool(options.cards, monsterTipo, monsterAtributo) : buildMergedPool(
      options.cards,
      [...activeIds],
    ...
    function pickBest(pool2, kind, phase, policyOpts) {
      const filtered2 = pool2.filter((c) => {
    ...
    function pickQuotaWithTierFallback(kind) {
      ...
      function pickFromPool(pool2, want, thematicOnly) {
        const atTier = pool2.filter((c) => {
    ...
    if (target === 5) {
      for (const staple of T5_STAPLE_SLUGS) {
        ...
        if (!card) {
          warnings.push(`Staple T5 ${staple.slug} n\xE3o encontrada no cat\xE1logo.`);
          continue;
        }
        ...
        if (added === 0) {
          warnings.push(
            `N\xE3o foi poss\xEDvel incluir staple T5 ${card.nome_pt || card.nome}.`
          );
        }
      }
    }
  ...
  var state = {
    ...
    savedDecks: [],
    lastSeed: null,
    selectedArchetypes: /* @__PURE__ */ new Set(["livre"])
  };
  var el = {
    ...
    pickerQ: document.getElementById("picker-q"),
  ...
  function syncDeckActionButtons() {
  ...
  function computeAveragePowerTier(entries) {
    let powerSum = 0;
    let n = 0;
    for (const e of entries) {
  ...
  function syncEditableFromResult(result) {
  ...
  function updateDeckWarnings() {
    ...
    el.warnings.textContent = msgs.join(" \xB7 ");
  }
  function applyEditableToDisplay() {
    const snap = buildEditableSnapshot();
    const deck = state.deck;
  ...
  function syncSidePanelBackdrop() {
    const anyOpen = !el.seedsPanel.hidden || !el.decksPanel.hidden || !el.pickerPanel.hidden;
  ...
  function setDecksPanelOpen(open) {
    if (open) {
      closeAllSidePanels();
      ...
      requestAnimationFrame(() => {
        for (const img of el.pickerList.querySelectorAll("img")) {
          ...
          if (!img.complete) {
            ...
            void img.decode?.().catch(() => void 0);
          }
        }
      });
    });
  }
  function setPickerPanelOpen(open) {
    if (open) {
  ...
  function deckWrapSelector(slug) {
    return `[data-slug="${CSS.escape(slug)}"]`;
  }
  function closeAllQtyPopoversDom() {
    ...
    for (const wrap of el.deckGrid.querySelectorAll(".deck-slot-wrap.is-qty-open")) {
      ...
      wrap.querySelector(".deck-slot-art")?.setAttribute("aria-expanded", "false");
    }
  }
  function getDeckWrap(slug) {
  ...
  function buildQtyPopoverEl(card, current) {
    ...
    minus.textContent = "\u2212";
    minus.setAttribute("aria-label", "Menos uma c\xF3pia");
    ...
    plus.textContent = "+";
    plus.setAttribute("aria-label", "Mais uma c\xF3pia");
    ...
    if (limit > 1) {
      ...
      for (let n = 1; n <= limit; n++) {
        ...
        btn.setAttribute("aria-label", `${n} c\xF3pia${n > 1 ? "s" : ""}`);
  ...
  function attachQtyPopover(wrap, slug) {
    ...
    wrap.querySelector(".deck-slot-art")?.setAttribute("aria-expanded", "true");
    wrap.appendChild(buildQtyPopoverEl(card, entry.copies));
  }
  function toggleQtyPopover(slug) {
  ...
  function updateDeckSlotDom(wrap, slug) {
    const card = state.bySlug.get(slug);
    const entry = entryForSlug(slug);
    if (!card || !entry) return;
    const artBtn = wrap.querySelector(".deck-slot-art");
    if (artBtn) {
      artBtn.title = `${card.nome_pt} \xD7${entry.copies}`;
    }
    let cop = wrap.querySelector(".copies");
    if (entry.copies > 1) {
      if (!cop) {
    ...
    const nameBtn = wrap.querySelector(".slot-name-btn");
    if (nameBtn) nameBtn.textContent = card.nome_pt || card.nome;
  }
  function setCardQty(slug, target) {
  ...
  function buildDeckSlotWrap(entry, index) {
    ...
    artBtn.title = `${c.nome_pt} \xD7${entry.copies}`;
  ...
  function renderDeckGrid(animate = true) {
  ...
  function autoSeedLabel(entry) {
    ...
    const theme = o.monsterTipo || o.monsterAtributo ? [o.monsterTipo, o.monsterAtributo].filter(Boolean).join("/") : archetypeIdsLabel(archIds);
    ...
    for (const s of seeds) {
      ...
      meta.innerHTML = escapeHtml(
        [
          ...
          `${archetypeIdsLabel(normalizeSeedArchetypeIds(o))}${o.monsterTipo ? ` \xB7 ${o.monsterTipo}` : ""}${o.monsterAtributo ? ` \xB7 ${o.monsterAtributo}` : ""}`,
  ...
  function autoDeckLabel() {
    const deck = state.deck;
    if (!deck) return `Deck ${(/* @__PURE__ */ new Date()).toLocaleDateString("pt-BR")}`;
    const theme = deck.monster_tipo || deck.monster_atributo ? [deck.monster_tipo, deck.monster_atributo].filter(Boolean).join("/") : deck.archetype_id;
    const snap = buildEditableSnapshot();
    return `${theme} \xB7 ${snap?.total ?? deck.total} cartas \xB7 ${formatSeedDate((/* @__PURE__ */ new Date()).toISOString())}`;
  }
  function renderSavedDecks() {
    const decks = state.savedDecks;
    el.decksStatus.textContent = decks.length ? `${decks.length} deck(s) guardado(s)` : "Nenhum deck guardado ainda.";
    const frag = document.createDocumentFragment();
    for (const d of decks) {
      ...
      meta.innerHTML = escapeHtml(
        [
          ...
          `${d.total} cartas \xB7 poder ${d.total_deck_power} \xB7 tier m\xE9d. ${d.average_power_tier}`,
  ...
  async function fetchSavedDecks() {
    ...
    const local = lsGet(LS_SAVED_DECKS, {
      updated_at: null,
      decks: []
    });
  ...
  async function deleteSavedDeck(id) {
    ...
    if (backendAvailable) {
      try {
        ...
        const json = await res.json();
        if (res.ok && json.ok) {
          state.savedDecks = json.decks?.decks || state.savedDecks.filter((d) => d.id !== id);
          renderSavedDecks();
          return;
        }
        el.decksStatus.textContent = `Falha: ${json.error || res.status}`;
  ...
  function loadSavedDeck(entry) {
    const meta = entry.meta;
    const metaIds = meta?.archetype_ids?.length ? meta.archetype_ids : meta?.archetype_id ? [meta.archetype_id] : ["livre"];
    ...
    state.deck = {
      ...
      monster_tipo: meta?.monster_tipo ?? void 0,
      monster_atributo: meta?.monster_atributo ?? void 0,
  ...
  function renderPicker() {
    ...
    for (let i = 0; i < max; i++) {
      ...
      meta.textContent = `${c.card_type} \xB7 ${c.raridade} \xB7 T${c.power_tier ?? "?"}`;
  ...
  function addCardFromPicker(slug) {
    if (!state.editableDeck) return;
    const card = state.bySlug.get(slug);
    if (!card) return;
    const current = state.editableDeck.counts.get(slug) || 0;
    if (!setCopyCount(card, state.editableDeck, current + 1, EDIT_OPTS)) {
      el.pickerStatus.textContent = `N\xE3o foi poss\xEDvel adicionar ${card.nome_pt || card.nome}.`;
      return;
    }
    applyEditableToDisplay();
    renderDeckGrid(true);
  ...
  async function boot() {
    ...
    if (saved) {
      const restoredIds = saved.archetypeIds?.length ? saved.archetypeIds : saved.archetype ? [saved.archetype] : ["livre"];
    ...
    ensureSidePanelBackdrop();
    window.matchMedia("(max-width: 720px)").addEventListener("change", syncSidePanelBackdrop);
    syncSaveSeedButton();
    syncDeckActionButtons();
